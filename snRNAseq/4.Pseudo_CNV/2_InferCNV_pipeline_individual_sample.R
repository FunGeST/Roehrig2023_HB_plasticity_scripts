...
### Choose sample to study
s = c("CHC2959N", "CHC2959T", "CHC2960T", "CHC3133T", "CHC3377N", "CHC3377T", "CHC3610T", "CHC3662T")

### Directory pathways
datadir <- "F:/Amelie-Datas/Pediatric tumors/3.Multiome ATAC RNA/2.snRNAseq/4.Pseudo_CNV/0.Input_data"
resdir <- paste0("F:/Amelie-Datas/Pediatric tumors/3.Multiome ATAC RNA/2.snRNAseq/4.Pseudo_CNV/1.Results/", s); if(!dir.exists(resdir)){dir.create(resdir)}

### Was there already an inferCNV run for this sample?
run_already_done = F # True if we want to use the previous inferCNV object as input
...
annotation <- annotation[grep(paste(c("CHC2959N", s), collapse="|"), annotation$V1),] # restrict to the cells from CHC2959N (that contains reference cells) and the sample of interest

for(t in c("CHC2959N", s)){
  annotation[grep(t, rownames(annotation)),"V2"] <- paste0(annotation[grep(t, rownames(annotation)),"V2"], paste0("_", t)) # add the sample name to the condition "normal" or "other"
}

annotation <- annotation[which(annotation$V2!="other_CHC2959N"),] # remove cells from CHC2959N that are not used as references
annotation$V1 <- NULL # the column containing the cell names is not necessary anymore since we have the rownames
...
genomic_position_file <- genomic_position_file[which(genomic_position_file$V2%in%paste0("chr", c(1:22, "X", "Y"))),] # keep only conventional chromosomes
...
###########################################
##### 1. CNV ESTIMATION WITH INFERCNV #####
###########################################
...
if(run_already_done==T){
  infercnv_obj <- readRDS(file.path(resdir, "run.final.infercnv_obj"))
  
}else{ # if there was no previous inferCNV run we have to create a new inferCNV object 
  infercnv_obj = CreateInfercnvObject(raw_counts_matrix=counts_matrix,
                                      annotations_file=annotation,
                                      delim="\t",
                                      gene_order_file=genomic_position_file,
                                      ref_group_names=c("normal_CHC2959N"),
...
infercnv_obj = infercnv::run(infercnv_obj,
                             ...
                             denoise=T,
                             HMM=T,
                             analysis_mode = "subclusters" # we want the algorithm to look for different subclones in the same sample 
)
