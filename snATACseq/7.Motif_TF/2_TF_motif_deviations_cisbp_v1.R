...
project_all <- addMotifAnnotations(ArchRProj = project_all, motifSet = "cisbp", name = "Motif_cisbp")
...
MotifMatrix <- getMatrixFromProject(
  ArchRProj = project_all,
  useMatrix = "Motif_cisbpMatrix")
save(MotifMatrix, file=file.path(resdir, "MotifMatrix_cisbp.RData"))
...
####################################
##### 3. ACCESS THE DEVIATIONS #####
####################################

dfVarDev <- getVarDeviations(project_all, name = "Motif_cisbpMatrix", plot = FALSE)
save(dfVarDev, file=file.path(resdir, "chromVAR_deviations_cisbp.RData"))

# plot Variability vs rank sorted annotations

df <- as.data.frame(dfVarDev)

pdf(file.path(resdir, "chromVAR_variable_motif_deviation_scores_cisbp.pdf"))
ggplot(df, aes(rank, combinedVars, color = combinedVars)) + 
  geom_point(size = 1) +
  ggrepel::geom_label_repel(
    max.overlaps=Inf,
    data = df[rev(seq_len(30)), ], aes(x = rank, y = combinedVars, label = name), 
    size = 1.5,
    nudge_x = 2,
    color = "black"
  ) + theme_ArchR() + 
...
saveArchRProject(ArchRProj = project_all, outputDirectory = getOutputDirectory(project_all))
